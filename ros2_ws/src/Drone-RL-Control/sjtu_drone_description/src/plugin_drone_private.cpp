  ...
  const ignition::math::v6::Vector3<double> & acceleration)
{
  ...
  // Initialize odometry origin using the drone's first Gazebo pose
  //if (!odom_initialized_) {
  //  initial_odom_x_ = pose.Pos().X();
  //  initial_odom_y_ = pose.Pos().Y();
  //  initial_odom_z_ = pose.Pos().Z();

  //  odom_initialized_ = true;
  //}

  // Express current position relative to the initial pose
  //odom.pose.pose.position.x = pose.Pos().X() - initial_odom_x_;
  //odom.pose.pose.position.y = pose.Pos().Y() - initial_odom_y_;
  //odom.pose.pose.position.z = pose.Pos().Z() - initial_odom_z_;
